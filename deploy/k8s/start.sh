...
echo ""
echo "🚀 Starting EcomApp environment..."
echo "Mode: $MODE"
...
IMAGES["configserver"]="${DOCKERHUB_USER}/ecom-config-server"
IMAGES["gateway"]="${DOCKERHUB_USER}/ecom-gateway"
IMAGES["user"]="${DOCKERHUB_USER}/user"
IMAGES["product"]="${DOCKERHUB_USER}/product"
IMAGES["order"]="${DOCKERHUB_USER}/order"
IMAGES["notification"]="${DOCKERHUB_USER}/notification"

SERVICES=("configserver" "gateway" "user" "product" "order" "notification")
...
kubectl create namespace ecomapp || true
...
helm upgrade --install infra helm/infra -n ecomapp
...
kubectl apply -k helm/ecomapp/secrets
helm upgrade --install ecomapp helm/ecomapp -n ecomapp
...
kubectl wait --for=condition=ready pod --all -n ecomapp --timeout=300s || true
...
kubectl port-forward svc/keycloak 8443:8080 -n ecomapp > portforward.log 2>&1 &
kubectl port-forward svc/gateway 8080:80 -n ecomapp > portforward.log 2>&1 &
